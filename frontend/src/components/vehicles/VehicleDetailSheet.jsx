...
export const VehicleDetailSheet = ({
  ...
  const [challanStatusDialog, setChallanStatusDialog] = useState({ open: false, challan: null, newStatus: '' });
  const [updatingChallanStatus, setUpdatingChallanStatus] = useState(false);
  ...
  const handleChallanStatusUpdate = async () => {
    const { challan, newStatus } = challanStatusDialog;
    if (!challan) return;

    setUpdatingChallanStatus(true);
    try {
      const response = await api.put(`/challans/${challan.id}/status`, {
        status: newStatus,
        payment_date: newStatus === 'Paid' ? statusPaymentDate : null
      });
      ...
      if (response.data.success) {
        ...
        if (localVehicleReport) {
          const updatedChallans = localVehicleReport.challans.map(c =>
            c.id === challan.id
              ? {
                ...c,
                status: newStatus,
                payment_date: newStatus === 'Paid' ? (statusPaymentDate || new Date().toISOString().split('T')[0]) : null
              }
              : c
          );
        ...
        // Also update the parent component's data if refresh callback exists
        if (onRefresh) {
          onRefresh();
        }

        setChallanStatusDialog({ open: false, challan: null, newStatus: '' });
        setStatusPaymentDate('');
      }
    } catch (error) {
      console.error('Error updating challan status:', error);
      toast.error(error.response?.data?.detail || 'Failed to update challan status');
    } finally {
      setUpdatingChallanStatus(false);
    }
  };
  ...
  return (
    <>
                ...
                {!localVehicleReport?.challans?.length ? (
                  <div className="text-center py-12 bg-slate-50 rounded-lg">
                    ...
                    <p className="text-xs text-slate-400 mt-2">
                      Click "Fetch Challans" to sync from Surepass or add manually
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {localVehicleReport.challans.map((challan) => {
                      const daysSinceIssue = Math.ceil((new Date() - new Date(challan.date)) / (1000 * 60 * 60 * 24));
                      const isOverdue = challan.status === 'Unpaid' && daysSinceIssue > 30;

                      return (
                        <Card key={challan.id} className={isOverdue ? 'border-rose-300 bg-rose-50/30' : ''}>
                          <CardContent className="p-4">
                            <div className="flex items-start justify-between">
                              <div className="flex items-start gap-3">
                                <div className={`p-2 rounded-lg ${challan.status === 'Paid'
                                    ? 'bg-emerald-100'
                                    : isOverdue
                                      ? 'bg-rose-200'
                                      : 'bg-rose-100'
                                  ...
                                  {challan.status === 'Paid'
                                    ...
                                    : <AlertTriangle size={20} className={isOverdue ? "text-rose-700" : "text-rose-600"} />
                                  }
                                </div>
                                <div>
                                  <div className="flex items-center gap-2 mb-1">
                                    <h4 className="font-semibold font-mono text-sm">{challan.challan_number}</h4>
                                    <Badge className={
                                      challan.status === 'Paid'
                                      ...
                                      {challan.status}
                                      {isOverdue && challan.status === 'Unpaid' && ' (Overdue)'}
                                    </Badge>
                                  </div>
                                    ...
                                    {challan.payment_date && (
                                      <>
                                        ...
                                        <span className="text-emerald-600">
                                          Paid: {new Date(challan.payment_date).toLocaleDateString()}
                                        </span>
                                      </>
                                    )}
                                  </div>
                                  {challan.phone_number && (
                                    <p className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                                      <Phone size={10} /> {challan.phone_number}
                                    </p>
                                  )}
                                </div>
                              </div>
                              <div className="text-right">
                                ...
                                {challan.status === 'Unpaid' && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="mt-2 border-emerald-500 text-emerald-600 hover:bg-emerald-500 hover:text-white"
                                    onClick={() => {
                                      setChallanStatusDialog({
                                        open: true,
                                        challan: challan,
                                        newStatus: 'Paid'
                                      });
                                      setStatusPaymentDate(new Date().toISOString().split('T')[0]);
                                    }}
                                  >
                                ...
                                {challan.status === 'Paid' && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="mt-2 border-amber-500 text-amber-600 hover:bg-amber-500 hover:text-white"
                                    onClick={() => {
                                      setChallanStatusDialog({
                                        open: true,
                                        challan: challan,
      ...
      {/* Challan Status Update Confirmation Dialog - OUTSIDE Sheet */}
      <AlertDialog
        open={challanStatusDialog.open}
        onOpenChange={(open) => {
          if (!open) {
            setChallanStatusDialog({ open: false, challan: null, newStatus: '' });
            setStatusPaymentDate('');
      ...
      >
        <AlertDialogContent className="z-[9999]">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {challanStatusDialog.newStatus === 'Paid' ? 'Mark Challan as Paid' : 'Mark Challan as Unpaid'}
            </AlertDialogTitle>
            <AlertDialogDescription className="space-y-3">
              <p>
                Are you sure you want to mark this challan as{' '}
                <strong className={challanStatusDialog.newStatus === 'Paid' ? 'text-emerald-600' : 'text-amber-600'}>
                  {challanStatusDialog.newStatus}
                </strong>?
              </p>

              {challanStatusDialog.challan && (
                <div className="bg-slate-50 p-3 rounded-md mt-3">
                  <p className="text-sm font-medium mb-2">Challan Details:</p>
                  <div className="space-y-1 text-sm">
                    <p><span className="text-slate-500">Number:</span> {challanStatusDialog.challan.challan_number}</p>
                    <p><span className="text-slate-500">Violation:</span> {challanStatusDialog.challan.violation_type}</p>
                    <p><span className="text-slate-500">Amount:</span> ₹{challanStatusDialog.challan.amount?.toLocaleString()}</p>
                    <p><span className="text-slate-500">Current Status:</span> {challanStatusDialog.challan.status}</p>
                  </div>
                </div>
              )}

              {challanStatusDialog.newStatus === 'Paid' && (
                <div className="mt-4">
              ...
              {challanStatusDialog.newStatus === 'Unpaid' && (
                <div className="mt-4 p-3 bg-amber-50 rounded-md border border-amber-200">
                  <p className="text-sm text-amber-800 flex items-center gap-2">
                    <AlertCircle size={14} />
                    This will remove the payment date and mark the challan as outstanding.
                  </p>
                </div>
              )}

              <p className="text-xs text-slate-400 mt-3">
                This change will be reflected in reports and driver risk scores.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={handleChallanStatusUpdate}
              disabled={updatingChallanStatus || (challanStatusDialog.newStatus === 'Paid' && !statusPaymentDate)}
              className={
                challanStatusDialog.newStatus === 'Paid'
            ...
            >
              {updatingChallanStatus ? (
                <>
                  <Loader2 size={14} className="animate-spin mr-2" />
                  Updating...
                </>
              ) : (
                `Yes, Mark as ${challanStatusDialog.newStatus}`
              )}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
